...
omit_flag_regulated_unkn_degr <- peak_data_orig %>%
  filter(peak_cd == "5")
...
# join omitted records and remove from working df
omit_flag <- bind_rows(omit_flag_dam_fail,
                       omit_flag_regulated_unkn_degr,
                       omit_flag_regulated,
                       omit_flag_affected
                       )

peak_data <- anti_join(peak_data_orig, omit_flag)
...
sites_unreg <- anti_join(sites_unreg_all, sites_unreg_lt_20,
                         by = join_by(site_no))
