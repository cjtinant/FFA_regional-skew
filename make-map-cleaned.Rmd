---
title: "Mapping USGS Stream Gages in the Great Plains Ecoregion"
author: "CJ Tinant"
date: "`r Sys.Date()`"
output: html_document
---

## Workflow Overview — Mapping USGS Stream Gages in the Great Plains Ecoregion

This RMarkdown document creates maps of USGS stream gages within the Great Plains ecoregion, focusing on sites with long-term, unregulated streamflow data. The workflow uses spatial data and USGS stream gage metadata to visualize and differentiate between all gages and those with peak flow data suitable for flood frequency analysis.

## Workflow Steps

1. Load required R packages for data manipulation, spatial analysis, and visualization.
2. Import site data:
   - `sites_all_in_bb.csv`: All USGS gages in the bounding box (BB).
   - `sites_all_peak_in_bb.csv`: Subset of gages with peak flow data and ≥20 years of unregulated records.
3. Prepare site data for mapping:
   - Select relevant columns.
   - Add grouping variables for plotting.
4. Create and customize the map:
   - Plot base map using `ggplot2`.
   - Overlay gage locations.
   - Differentiate sites by data type (all gages vs. peak flow gages).
5. Export the final map for reporting or publication.
