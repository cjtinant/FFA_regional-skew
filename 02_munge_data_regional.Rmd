...
```{r 01_import_peak-flow-station-metadata_eco_shapefile}
...
```{r 04_filter_peak-flow_sites}

sites_st_n <-sites_all_n %>%
  filter(site_tp_cd == "ST")

sites_st_s <-sites_all_s %>%
  filter(site_tp_cd == "ST")

# get a list of all peak flow data in bounding box
sites_pk_n <-  whatNWISdata(
  siteNumber = sites_st_n$site_no,
  service = "pk"
  )

sites_pk_s <-  whatNWISdata(
  siteNumber = sites_st_s$site_no,
  service = "pk"
  )

sites_peak_all <- bind_rows(sites_pk_n, sites_pk_s)

# convert stations into a spatial format (sf) object
sites_peak_all <- st_as_sf(sites_peak_all,
