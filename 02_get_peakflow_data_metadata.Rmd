...
#     The function returns the modified sf object with additional columns for
...
```{r 02_process_geometries}

eco_l3 <- process_geometries(eco_l3)

```



```{r 04_visually_check_results}
...
# Safely calculate centroids for valid geometries
eco_l3$centroid <- ifelse(st_is_valid(eco_l3$geometry), st_centroid(eco_l3$geometry), NA)
...
# Plotting the map with text labels
ggplot() +
  geom_sf(data = eco_l3,
          aes(fill = na_l2name)
          ) +
  geom_text(data = eco_l3,
            aes(x = text_x,
                y = text_y,
                label = na_l2name
                ),
...
# Get level 2 ecoregion names based on level 1 ecoregions:
#   * Great Plains,
...
# Get level 3 ecoregion names based on level 2 ecoregions:
